...
with tab_team_reports:
    ...
    def compute_performance_badge(row, df_all):
        """
        Assigns an overall rating badge based on a Z-score approach,
        with corrected sign logic for defensive metrics
        and updated thresholds.
        """
        t_avgs, t_stdevs = compute_tournament_stats(df_all)
        metrics = get_default_metrics()

        # For these metrics, a lower value is better => invert
        lower_is_better = ["DEF EFF", "KP_AdjD", "TO/GM"]
        z_vals = []

        for m in metrics:
            if (m in row) and (m in t_avgs) and (m in t_stdevs):
                mean_ = t_avgs[m]
                std_ = t_stdevs[m] if t_stdevs[m] > 1e-12 else 1e-6
                val = row[m]
                z = (val - mean_) / std_

                # Invert certain defensive metrics
        ...
        # Slightly revised thresholds to avoid “strong” teams
        # being labeled as WEAK due to one or two outlier metrics
        if avg_z >= 1.0:
            return {"text": "ELITE", "class": "badge-elite"}
        elif avg_z >= 0.3:
            return {"text": "SOLID", "class": "badge-solid"}
    ...
    if selected_team_reports:
        ...
        if team_data.empty:
            ...
            # Layout columns for the overview
            ...
            with colA:
                ...
                if "SEED_25" in team_data.columns and not pd.isna(team_data["SEED_25"].iloc[0]):
                    ...
                    # simple color approach for seeds
                ...
                team_initial = selected_team_reports[:1].upper()

                # Open the card container + team-info block
                st.markdown(f"""
                <div class="team-card">
                ...
                # Now insert the "stat bubbles" in a loop
                key_stats = []
                if "WIN% ALL GM" in team_data.columns:
                    win_pct = round(team_data["WIN% ALL GM"].iloc[0], 2)
                    key_stats.append(("WIN% ALL GM", f"{win_pct*100:.0f}%", "#333333"))
                ...
                # Close the flex container, the team-info block, and the card
                st.markdown("""
            ...
            with colB:
                ...
                def get_interpretive_insights(row, df_all):
                    ...
                    for metric in get_default_metrics():
                        if metric in row:
                            ...
                            team_val = row[metric]
                            z = (team_val - mean_val) / std_val
                            # Invert for certain defensive metrics
                            if metric in ["DEF EFF", "TO/GM", "KP_AdjD"]:
                                z = -z
                ...
                if team_insights:
                    ...
                    for insight in team_insights:
                        # insight is like "**KP_AdjO** | Clear strength."
                        # separate out the metric name from the comment if desired
    ...
    if selected_opponent and selected_opponent != selected_team_reports:
        ...
        if opp_data.empty:
            ...
            # -----------------------------------------
            # HEAD-TO-HEAD STATS COMPARISON EXPANDER
            # -----------------------------------------
            with st.expander("Head-to-Head Stats Comparison"):
                ...
                lower_is_better = {
                    "KP_Rank": True,
                    "KP_SOS_AdjEM": False,  # Usually, a higher SOS is more challenging, so do NOT invert
                    "DEF EFF": True,
                    "OPP eFG%": True,
                    "TO/GM": True,
                }
                ...
                def colorize_row(row_):
                    metric = row_["METRIC"]
                    invert_ = lower_is_better.get(metric, False)
                    styles = []
                ...
                # Combine interpretive insights
                team_insights_str = "".join(
                    f"<li>{ins}</li>" for ins in get_interpretive_insights(row_team, df_main)
                )
                opp_insights_str = "".join(
                    f"<li>{ins}</li>" for ins in opp_insights
                )

                st.markdown(f"""
                <div style='margin-top:15px;'>
                    <p><strong>Win Probability</strong>: {selected_team_reports} has a {pA*100:.1f}% chance 
                       to beat {selected_opponent}.</p>
    ...
    if selected_opponent and selected_opponent != selected_team_reports:
        ...
        if opp_data.empty:
            ...
            with st.expander("Head-to-Head Stats Comparison"):
                ...
                # Combine interpretive insights
                # (We can reuse get_interpretive_insights for the team row, or define anew.)
                team_insights_str = "".join(
                    f"<li>{ins}</li>" for ins in get_interpretive_insights(row_team, df_main)
                )
